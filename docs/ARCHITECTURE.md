...
- [x] **Grasp physics: physical (friction) grip.** The DetachableJoint virtual attach is gone. The gripper is a position-servo (gz_ros2_control velocity-based) drive; the grasp skill detects a blocked close and retains a small over-close command until release, maintaining the normal force for friction. Fingertip collision volumes match the asymmetric rendered meshes, and the grasp target is calibrated from those visible pad faces so the claws do not appear inside an object. Verified: cube physically transported to tray, postcondition passes.
