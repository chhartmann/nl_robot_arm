...
- [x] **Grasp: grasp-fix plugin + firm-close fallback.** The `nlra_gz_grasp_fix` gz plugin (Gazebo Harmonic port of gazebo_grasp_fix) creates a fixed detachable joint from the palm to a dynamic object once a fingertip contact sensor touches it while the knuckle is closing (both-sided is preferred; a persistent one-sided contact is accepted so curved/compliant objects like cylinders, which only ever register one fingertip at the nominal close angle, still attach). The joint is removed only after the knuckle actually opens, so transient contact-manifold flicker during transfer cannot drop an object. The grasp skill detects a blocked close and retains a low over-close command until release; if the position servo reaches the calibrated target before the (soft) contact stalls — which happens for cylinders and edge contacts — the orchestrator re-issues a firm full-close so the fingers press until they physically block (an empty close still reaches target with no stall, so this creates no false positives). Fingertip collision volumes match the asymmetric rendered meshes, and the grasp target is calibrated from those visible pad faces. Verified: red_cube, green_cube, yellow_cylinder, and orange_block all pick on the first try; cube and cylinder physically transported to tray, postcondition passes.
